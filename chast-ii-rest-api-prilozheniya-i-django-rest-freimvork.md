...
Приложение "проект" является менеджером или управляющим, позволяющим сторонним/самописным приложениям взаимодействовать с Django библиотеками. Эти взаимодействия возможны благодаря классу `AppConfig` \([документация](https://docs.djangoproject.com/en/1.10/ref/applications/#django.apps.AppConfig), [источник](https://docs.djangoproject.com/en/1.10/_modules/django/apps/config/#AppConfig)\). Файл с настройками знает о других приложениях и их функциях, потому что использует `AppConfig`, который обычно находится в файле `app.py` по умолчанию. После добавления названия каталога в  INSTALLED\_APPS, сначала происходит поиск файла `app.py` в этом каталоге для настройки приложения \(класс `AppConfig` может находиться в другом файле, но мы предполагаем, что используется структура, по умолчанию применяемая командой `manage.py startapp`\).

---

Сейчас мы создадим другое приложение, используя команду `startapp` \([источник](https://docs.djangoproject.com/en/1.10/ref/django-admin/#startapp)\). Структура приложения, используя команду `startapp` будет иметь вид:
...
* обработки данных , используя [обобщенные представления](http://www.django-rest-framework.org/api-guide/generic-views/) и [ViewSets](http://www.django-rest-framework.org/api-guide/viewsets/);
...
**Ссылки с описанием представлений-классов, ViewSetов и сериализаторов , предоставляемых DRF, также можно найти на сайте **[**Classy Django REST Framework**](http://www.cdrf.co/)**.**
...
Теперь мы можем манипулировать моделью `User` с помощью REST! Если Вы хотите узнать больше о создании URL-адресов и получить обобщенное представление о регулярных выражениях, воспользуйтесь [этой ссылкой](https://docs.python.org/3/library/re.html).
...
После того как сервер стартовал, мы можем просмотреть API Root в браузере! Перейдите по адресу [http://localhost:8000/](http://localhost:8000/) и Вы должны увидеть эту страницу!
...
Этот проект со всеми разделами можно найти на GitHub по этой [ссылке](https://github.com/djstein/modern-django).
