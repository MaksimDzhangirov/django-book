...
![](/images/part1/vic.png)

\*Очередная победная музыка в Вашей голове\*

На данный момент мы закончили настройку параметров! Надеюсь теперь Вы намного лучше понимаете как взаимодействуют друг с другом все части Django и Вас больше не пугает API для настроек! При установке дополнительных сторонних приложений, мы будем добавлять связанные с ними настройки в соответствующий файл настроек.

### 4. Используем наши новые настройки

Чтобы использовать наши только что созданные, новые настройки, нам необходимо отредактировать `manage.py`. В строку:

```
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "config.settings")
```

добавьте `.local` к `config.settings` и убедитесь, что она теперь имеет следующий вид:

```
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "config.settings.local")
```

Теперь проект будет использовать правильные настройки для локальной разработки! Позднее мы внесем дополнительные изменения в настройки для продакшен и тестового окружения.

### 5. Создаём файл .env

Вспомним пункт _3-С. Импортируем переменные окружения_, где мы обсуждали как django-environ ищет файл `.env`. Теперь давайте создадим его! В каталоге `modern-django`, мы создадим файл и назовём его `env.example`. В этом файле будут храниться переменные окружения, необходимые для проекта. Когда мы будем использовать этот файл для продакшен окружения, мы скопируем и переименуем его в `.env`, поместив в него необходимую секретную информацию \(например, настоящий `SECRET_KEY`\). Таким образом, `env.example` будет отслеживаться в git, а `.env` - нет.

Сейчас создайте файл `env.example`, а затем скопируйте и переименуйте его:
...
Пока что мы ничего не будем добавлять в эти файлы, поскольку их необходимо будет изменять только при переходе в продакшен окружение.

### **6. Обновляем установленные пакеты и запускаем сервер!!**

Теперь, после рефакторинга файла requirements.txt и настроек, пора запустить проект и убедиться в его работоспособности.

Сначала установите `django-environ` в Ваше виртуальное окружение, используя файл `local.txt`. Перед установкой убедитесь, что виртуальное окружение активировано, иначе пакет установится в каталог с pip, который используется операционной системой, а не виртуальным окружением:
...
Теперь пора запустить наш сервер! Используйте команду `runserver` из файла `manage.py`. По умолчанию сервер запускается по адресу [http://127.0.0.1:8000/](http://127.0.0.1:8000/)
...
Перейдите по адресу [http://127.0.0.1:8000/](http://127.0.0.1:8000/) или [http://localhost:8000/](http://localhost:8000/) и Вы должны увидеть:![](/images/part1/worked.png)“It’s alive!” — фраза доктора Франкенштейна \(фильм Молодой Франкенштейн, 1974\)

**Замечание:** Вероятно Вы заметили сообщения Django о не выполненных миграциях. Мы займемся ими в следующий раз.

### 7. Фиксируем изменения и отправляем на удаленный сервер

Как обычно, не забудьте использовать команду `git add` для добавления файлов, `commit` - для фиксации изменений и `push` - для отправки их в Ваш GitHub репозиторий. Иначе проделанные выше изменения не сохранятся на удаленном сервере.
